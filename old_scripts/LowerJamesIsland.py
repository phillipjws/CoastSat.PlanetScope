...
def extract_and_filter_shorelines(settings, outputs):
    ...
    shoreline_data = extract_shorelines(outputs, settings, del_index=False, rerun_shorelines=False, reclassify=False)
...
def estimate_beach_slope(settings, sl_csv, dates_sat, dates_ts, tides_sat, tides_ts):
    ...
    settings_slope = {
        ...
        'delta_slope': 0.005,
        'n0': 50,
        'freq_cutoff': 1. / (24 * 3600 * 30),  # 1 month frequency
        'delta_f': 100 * 1e-10,
        'prc_conf': 0.05,
        'plot_fig': True,
    }
    ...
    # Convert output dates to datetime and filter them by date range
    output_dates = [pytz.utc.localize(datetime.strptime(date_str, '%Y-%m-%d')) for date_str in output['date']]
    idx_dates = [settings_slope['date_range'][0] < _ < settings_slope['date_range'][1] for _ in output_dates]

    # Ensure consistent array lengths
    dates_sat = [output_dates[_] for _ in np.where(idx_dates)[0]]
    tides_sat = tides_sat[np.where(idx_dates)[0]]
    
    # Apply filtering on sl_csv and ensure lengths match
    for key in sl_csv.keys():
        sl_csv[key] = sl_csv[key][idx_dates]

    SDS_slope.plot_timestep(dates_sat)
    plt.gcf().savefig(os.path.join(fp_slopes, '0_timestep_distribution.jpg'), dpi=200)

    settings_slope['n_days'] = 2
